...
def extract_soup_pirate_data(pirate_name):
    BASE_URL = "https://onepiece.fandom.com/wiki/"
    headers = {"User-Agent": "Mozilla/5.0"}
    url = f"{BASE_URL}{pirate_name}"
...
# extracting all straw hat pirates data
for pirate in straw_hat_pirates:
    pirate_data_dict = {}
    soup = extract_soup_pirate_data(pirate)
    ...
    pirate_data_dict["Description"] = clean_text(description)
        ...
        for item in data_items:
            ...
            if label_tag and value_tag:
                ...
                if label.lower() in label_exclude:
                    ...
                    pirate_data_dict[label] = parse_value_with_context(value)

                elif label.lower() in ["height"]:
                    pirate_data_dict[label] = parse_value_height(value_tag)
                    ...
                    pirate_data_dict[label] = bounty_entries
                
                elif label.lower() == "alias":
                    aliases = value.split()
                    pirate_data_dict[label] = aliases

                else:
                    pirate_data_dict[label] = value

                all_characters[character_name] = pirate_data_dict

with open("straw_hat_pirates_final.json", "w", encoding="utf-8") as f:
